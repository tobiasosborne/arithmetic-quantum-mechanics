...
have passed their capped reviews, together with the Fock completion, prime
tensor assembly and represented Bost--Connes control. All fifteen bootstrap
propositions are proved. Finite probes check the formulas; arbitrary-rank
and infinite-dimensional conclusions rest on the written proofs. The global,
modular and spectral design decisions remain open.
\provenance{SP-WEYL,SP-EGOROV,SP-TENSOR,SP-LREL,SP-COMPACT,SP-STAB-REL,SP-SCALAR,SP-CP,SP-SUM,SP-TRACE,SP-FROB,SP-SUBSYS,SP-FOCK,SP-PRIME,SP-BC-CONTROL}{Structured bootstrap proofs}{Exact finite controls at their declared scope}{Stage-1, relation, process, sum, arithmetic and completion adjudications}
...
that compose in reverse. The completion controls now supply bounded Fock
maps, the chosen prime product state and a represented arithmetic benchmark.
Characteristic-two and scalar-retaining source interfaces still precede the
...
The Fock prescription takes actual contractions, and its exponential
comparison uses coherent Hilbert direct sum. The symplectic direct-sum
comparison established here instead uses Hilbert tensor product. A later
additive construction must name which product each comparison preserves;
no Fock assignment on scalar classes is prescribed. Prime assembly also
retains chosen reference states. Faithfulness of a state, faithfulness of
its GNS representation, and whether its reference vector is separating
are distinct questions; cyclicity alone is the current tensor-assembly
claim, while the modular comparison has additional hypotheses.
\provenance{D1708,D1711,D1713,SP-FOCK,SP-PRIME}{Specified completion data and structured proofs}{Exact finite/symbolic controls at their declared scope}{theory/verdicts/phantasm-completions-adjudication.md}
...
\end{scope}
\provenance{D1712}{Stipulation; SP-BC95, SP-CM04, SP-CM08; structured represented-control proof}{Exact symbolic arithmetic controls}{Reuses: none; completion adjudication}
