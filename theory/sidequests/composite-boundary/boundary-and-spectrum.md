...
2026-09-09. Root prover. CMP-BOUNDARY, CMP-FOURIER, CMP-TRACE are PROVED
within their stated scopes after capped review and mechanical repair.
Definitions: ../../../definitions.md, D1602--D1607; companion construction supplies
...
weights and conditional moving-sector boundary.
  ...
  <2>3. ASSUME additionally n>1 for moving-sector conditioning.
  On d>1, first-order unnormalized weights are s phi(d)/d.
  ...
  <2>4. For n=1 the moving sum is empty, has zero weight at every h,
  and has no conditional state. The all-period mixture still has total
  success t^(-s)c_1(t^s)=1, using C defined for n>=1. Uniform
  randomization uses n copies of R^j/n for each j; its squared-amplitude
  sum is I and its channel is the declared (1/n)sum_j Ad(R^j).
  For n>1 the raw ordinary endpoint retains only d=1, while moving-sector
  conditioning of the first nonzero grade retains the stated matrix blocks.
