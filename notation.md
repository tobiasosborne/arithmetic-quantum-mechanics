...
| `P_r`, `P_r^L`, `U_pi`, `Sym^r H`, `Gamma_s(H)`, `Omega`, `N_H`, `jmath_H`, `jmath_K`, `Exp^(alg)_(H,K)`, `Exp_(H,K)`, `odot` (Fock) | symmetrizer, permutation action, symmetric sector, Hilbert Fock sum, vacuum and particle number; coordinate inclusions, directed algebraic exponential candidate, its bounded extension when it exists, and algebraic tensor product over C; the one-mode polynomial completion is already F_bos in D1010 | D1708 |
...
| `P`, `A_P`, `iota_QP`, `A_pr`, `varphi_pr`, `d_p`, `rho_p` (prime tensor assembly) | finite prime set, finite tensor algebra, increasing-prime identity-insertion embedding, including the empty stage, norm completion, product reference and chosen local dimensions/densities | D1711 |
