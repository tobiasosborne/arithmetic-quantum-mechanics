...
\texttt{available}: local one \(\mathbb F_3\) residue site; the database
...
\texttt{available}: local AQM-77 cotangent example only at
\((u,v)\); standard Artin-row status is \texttt{blocked}. &
\texttt{available}: local residue \(\mathbb F_3\) in AQM-77; no
...
\addlinespace
\(\mathbb Z/4\mathbb Z\) &
\texttt{available}: AQM-77 local cotangent comparison only at
\((2)\). &
\texttt{available}: local residue \(\mathbb F_2\) in AQM-77; the database
