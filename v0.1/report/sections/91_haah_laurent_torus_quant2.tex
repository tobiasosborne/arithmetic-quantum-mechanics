...
% SHARD-SUMMARY: Identifies the \(p=2\) finite periodic bridge where the AQM-90 Cayley square becomes Haah's toric-code matrix.
% SHARD-SUMMARY: Records proposal-level lessons for future arithmetic-field Weyl-Heisenberg label constraints.
...
D5&:&R_{G,p}=\mathbb F_p[X^{\pm1},Y^{\pm1}]/(X^{N_x}-1,Y^{N_y}-1).\\
D6&:&\text{For the AQM-90 comparison, }p=2,\ R_G=R_{G,2}.\\
D7&:&P_G=R_G^4\cong
\mathbb F_2^{G\times\{x,y\}}{}_q\oplus
\mathbb F_2^{G\times\{x,y\}}{}_p.\\
L1&:&\sigma_\square\text{ reproduces the AQM-90 square CSS subgroup.}\\
L2&:&\sigma_\square^\dagger\lambda_2\sigma_\square=0
...
Haah's starting point is translation invariance.  For prime-qudit labels over
\(\mathbb F_p\), a lattice with translation group \(\mathbb Z^2\) gives the
group algebra
\[
...
\]
AQM-90 uses this \(G\) as a finite vertex set.  The corresponding generic
prime-qudit periodic translation quotient is
\[
  R_{G,p}=
  \mathbb F_p[X^{\pm1},Y^{\pm1}]/(X^{N_x}-1,Y^{N_y}-1).
\]
As an \(\mathbb F_p\)-vector space, \(R_{G,p}\) has the basis
...
This is the finite bridge.  It is not the claim that \(R_{\rm tr}\) is the
coordinate ring of \(\Spec A_T\).  It is the finite periodic claim that the
translation quotient acts on a site-indexed label module.  The literal
comparison with AQM-90's qubit edge module is the specialization \(p=2\),
written \(R_G=R_{G,2}\) below.
...
\]
is the Haah form of the AQM-90 identity \(\partial_1\partial_2=0\).  Over the
infinite translation ring \(R_{\rm tr}\), the corresponding toric map
\(\sigma_{\rm tr}\) has excitation map
\(\epsilon_{\rm tr}=\sigma_{\rm tr}^\dagger\lambda_2\), and the source-local
toric exactness statement is
\[
  \ker\epsilon_{\rm tr}=\operatorname{im}\sigma_{\rm tr},
\]
as recorded in the local Haah thesis locator
\path{references/lattice_stabilizer_codes/haah_1305_6973_source/alg-theory.tex},
lines 1515--1567.  On the finite periodic quotient \(R_G\), this shard does
not assert that equality without a local finite-periodic check.  The object to
record is instead the possible logical quotient
\[
  \ker\epsilon_\square/\operatorname{im}\sigma_\square .
\]
For AQM purposes, the source-local exactness equation is a sharper anchor and
proposal template than bare isotropy, while the finite-periodic quotient is a
quantity to measure.
...
The second lesson is exactness discipline.  AQM-90 verified the square
boundary identity.  Haah teaches us to ask for the pair
\[
