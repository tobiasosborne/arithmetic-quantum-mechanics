...
label maps.  The sign convention is also the one bridged explicitly in
Section~\ref{sec:prime-field-arithmetic-fields-qudit-paulis}: relative to
those source formulas, the local \(X\)-coordinate is the negative of the
source shift exponent and the local \(Z\)-coordinate is unchanged.
...
\]
The \(\mathbb F_2\) factor uses the unphased projective Weyl representatives
from Section~\ref{sec:prime-field-arithmetic-fields-qudit-paulis}.  Adjoining
the conventional \(\mu_4\) phases changes the Hermitian representatives, for
example \(Y=iXZ\), but not the label group, operator span, or tensor-product
Hilbert-space conclusion below.
