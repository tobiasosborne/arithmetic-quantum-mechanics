...
% SHARD-TITLE: \(\mathbb F_2\) Qubit Examples for the de Rham CSS Prototype
% SHARD-SUMMARY: Computes five characteristic-two examples for the finite de Rham CSS prototype, with 5, 6, 8, 9, and 10 auxiliary \(C^1\)-qubits.
...
% SHARD-SUMMARY: Lists explicit prototype \(C^1\)-qubits and the selected \(X\)- and \(Z\)-stabilizer generators for each example.
% SHARD-KEYWORDS: F2, qubit, de Rham, CSS, stabilizer, Kahler differentials, characteristic two, nilpotent
\section{\texorpdfstring{\(\mathbb F_2\)}{F2} Qubit Examples for the de Rham CSS Prototype}
\label{sec:derham-css-f2-qubit-examples}
\subsection{Status, Sources, and Characteristic-Two Warning}
This shard continues the finite chain-complex prototype of AQM-54 and AQM-55
with \(k=\mathbb F_2\).  Its qubits are auxiliary \(C^1\)-basis qubits, not
the Hilbert carriers obtained by quantising the pointwise
\(T_PX\oplus T_P^*X\) label spaces.  The source anchors are unchanged: Stacks
algebra lines 33793--33872, 34103--34120, 34310--34340, and 34388--34524.
AQM-08 supplies the CSS/stabilizer dictionary, and \texttt{CONVENTIONS.md} (ao)
names the \(\texttt{de\_rham\_css\_test}\).
...
\end{array}
\]
For a prototype \(C^1\)-basis vector \(e_i\), write \(X_i=X(e_i)\) and
\(Z_i=Z(e_i)\).  All stabilizers below have \(+1\) phase; the ring calculation
fixes only the support spaces.
...
\]
Then \(C^1=\Omega^1_{A_0/k}\cong k^6\), so the prototype has six auxiliary
qubits.  The
...
\]
Thus \(C^1\cong k^{10}\), giving ten auxiliary prototype qubits.  The same
...
\[
  ...
  e_5=x\,dy=y\,dx.
\]
The prototype has five auxiliary qubits.  The derivation on the basis of \(A_2\)
is
\[
...
\end{array}
\]
Thus \(C^1\cong k^9\), so the prototype has nine auxiliary qubits.  The image
of \(d_0\) is
\[
...
\subsection{End Summary: Prototype Qubits and de Rham CSS Stabilizers}
{\scriptsize
\[
...
The prototype qubit lesson is sharper than the odd-characteristic lesson: in
characteristic two, quadratic nilpotent equations can leave more global
one-form directions alive.  These \(C^1\)-qubits are not, without further
fibrewise Weyl analysis, the canonical tangent-Weyl qubits of the scheme.
