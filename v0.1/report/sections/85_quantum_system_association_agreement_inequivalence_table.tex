...
% SHARD-SUMMARY: Records only locally witnessed agreements and inequivalences, with unsupported comparisons left blocked or unknown under the local status-token convention.
...
their meanings from convention \texttt{(au)}.  AQM-84 boundary categories
such as \emph{generic-sector proposal} and \emph{completion proposal} are
prose classifiers for witness and gap columns, not status tokens in this
table.  They are not finite Weyl-site claims.
...
pairing, radical quotient, and Weyl step.  AQM-67 has only formal basis labels;
comparison remains \texttt{unknown} because no map compares the full outputs. &
AQM-67, AQM-71 \\
\addlinespace
...
the full CCR/CAR or Grassmann-Weyl outputs with the finite grammar; comparison
remains \texttt{unknown}. &
AQM-70, AQM-72 \\
\addlinespace
...
at every product point, so the cotangent output is \texttt{degenerate}. &
...
or formal infinite-volume layers &
\texttt{available}. &
AQM-84 classifies finite supports, algebraic quasi-local colimits,
reference-vector Hilbert representations, and formal infinite-volume profiles
as separate boundary categories.  Their comparison is \texttt{blocked}; formal
infinite-volume operators still need a completion convention. &
AQM-84 \\
\addlinespace
Algebraic generic-fibre layers versus completed infinite boundary layers &
\texttt{available}. &
The named algebraic sectors are locally anchored: convention \texttt{(z)}
records the algebraic \(K=\mathbb F_q(t)\) Weyl layer, and AQM-45 records the
optional \(\mathbb Q\)-generic sector for \(\Spec\mathbb Z\).  Completion
comparisons remain \texttt{blocked}; these are global generic-fibre layers, not
closed-point finite sites or analytic/local, adelic, profinite, \(C^*\), or
infinite-product completions. &
AQM-45, AQM-84, convention \texttt{(z)} \\
\bottomrule
\end{tabular}
\end{center}
...
\(\mathbb F_3\) have \texttt{degenerate} zero output. &
...
Common-field vector-space readings are \texttt{not\_applicable}; cotangent and thickened comparisons are \texttt{unknown} or \texttt{blocked} by row; further quotient-collapse rows are blocked. &
...
\texttt{available}: finite closed supports, algebraic quasi-local
closed-point nets, selected incomplete tensor representations where recorded,
and the algebraic generic sectors for \(K=\mathbb F_q(t)\) and
\(\mathbb Q\) where separately anchored. &
\texttt{blocked}: curve-generic sectors without a recorded convention and
analytic/local, adelic, profinite, \(C^*\), and infinite-volume operator
completions.  AQM-84 proposal categories are gap prose, not status tokens. &
