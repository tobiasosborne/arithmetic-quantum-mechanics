...
This explains the impedance mismatch. The cellular supercharge
\(Q_{\mathrm{cell}}\) acts on classical cochains and computes the two
topological bits of the torus. The ghost-supercharge \(Q\) acts on the quantum
Hilbert space enlarged by auxiliary fermions and has the stabilizer Hamiltonian
as its anticommutator. These are not the same operator, but they are two levels
of one construction: the chain complex supplies the CSS supports; the
ghost/Koszul lift turns the corresponding commuting stabilizer constraints
into a supersymmetric Hamiltonian.
...
The run output is the CSV \path{toric_chain_ghost_unification.csv} in the
\path{2026-05-24-toric-chain-ghost-unification} run bundle.
