...
% SHARD-SUMMARY: Gives checked Weyl-label targets for Steane, Reed-Muller, Shor, and toric-code source conjectures.
...
L=(\operatorname{im}\delta^0\oplus0)+(0\oplus\operatorname{im}\partial_2)
\text{ is isotropic.}\\
T1&:&\text{Steane, Reed-Muller, Shor, toric are tested below by their }L.
\end{array}
\]

\subsection{Why a Bare Finite Ring Is Not Enough}

Finite rings are Artinian examples.  The Stacks Artinian source records that
an Artinian ring has finitely many maximal ideals, all primes are maximal, and
the spectrum is finite discrete after decomposition into local factors.  AQM-40
gives the concrete reduced case
\[
...
Bombin--Martin--Delgado's local source says Shor's original \([[9,1,3]]\) code
belongs to a homological family \([[d^2,1,d]]\) at \(d=3\), but this shard does
not yet reconstruct their surface 2-complex.
...
AQM-05/AQM-06 validated toric instance.
...
The evidence separates the four routes.  Steane is a punctured Boolean
three-space plus a homogeneous degree-one selection rule.  Reed-Muller is the
Boolean cube with its degree filtration; orthogonality is checked above, while
distance and gate claims still need sources or runs.  Shor has a sourced
incidence-chain target but still needs the natural homological
\([[d^2,1,d]]\) surface complex reconstructed locally.  Toric is the constant
group scheme plus the Cayley square complex; its remaining issue is justifying
that group/incidence input as arithmetic rather than only combinatorial.  In
all four cases the final object is a selected Weyl-label subgroup \(L\), hence
a selected subgroup \(\pi^{-1}(L)\) of the Weyl-Heisenberg group.
