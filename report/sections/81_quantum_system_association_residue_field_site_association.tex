...
\addlinespace
\(\mathbb Z/4\mathbb Z\) &
\texttt{blocked/database}: AQM-77 locally uses the point \((2)\) and residue
\(\mathbb F_2\) for cotangent comparison, but the finite-ring residue helper
does not certify the row. &
\texttt{not\_claimed}: no general Step 17 database-backed residue-site row is
imported here. &
...
\texttt{available locally}: AQM-34/AQM-80 record one reduced residue site
