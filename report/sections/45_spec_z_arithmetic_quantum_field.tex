...
% SHARD-SUMMARY: Defines the internal integer residue-translation action and proves its profinite, nonlocal character.
% SHARD-KEYWORDS: Spec Z, prime qudits, generic point, incomplete tensor product, translation, profinite integers
...
This shard extends \texttt{CONVENTIONS.md} (u)--(y) and (ai).  The
source-local scheme facts are from
...
functoriality, quasi-compact opens, and generic points.  AQM-28 supplies the
finite closed-residue Weyl net, and AQM-42 supplies finite-support dynamics.
...
L3&:&\text{Nonempty opens are }\{\eta\}\cup\{x_\ell:\ell\notin S\}
      \text{ with }S\text{ finite}.\\
D2&:&E_{\rm cl}(U)=\bigoplus_{x_\ell\in U}\mathbb F_\ell^2,\quad
      \mathcal A_{\rm cl}(U)=\varinjlim_{S\Subset U_{\rm cl}}\bigotimes_{\ell\in S}M_\ell(\mathbb C).\\
...
T3&:&\alpha:\mathbb Z\curvearrowright\mathcal A_{\rm cl}(X)
      \text{ is faithful and extends profinitely.}\\
...
\emph{Proof.}
This is AQM-28 applied to the finite-residue closed points of \(X\).  If
...
Because every nonempty open contains \(\eta\), the generic-sector local rule is
\[
  \mathcal A_{\rm gen}(U)=
...
extends to the compact residue product \(\prod_\ell\mathbb F_\ell\), and hence
to \(\widehat{\mathbb Z}\) after reducing each \(p\)-adic coordinate modulo
\(p\).
...
algebra sees the profinite completion.  The global translation by \(1\) is an
automorphism of the quasi-local algebra but not a unitary in the
