...
dictionary is AQM-08.  The real CCR comparison is AQM-19.
\end{sloppypar}
...
\[
\begin{array}{rcl}
D1&:&X=\Spec A,\quad P\in X,\quad K=\kappa(P).\\
D2&:&V_P^*=T_P^*X=\Omega^1_{A/k}\otimes_AK.\\
D3&:&V_P=T_PX=\operatorname{Hom}_K(V_P^*,K).\\
...
D8&:&E_S=\bigoplus_{P\in S}E_P,\quad
      ...
      \text{ for finite }S.\\
D9&:&L\subset E_S\text{ isotropic } \Rightarrow
      \{W(e):e\in L\}\text{ is a commuting family after phases}.\\
D10&:&\text{Chosen phases for }L\text{ define a stabilizer eigenspace}.\\
...
Let \(V=V_P=T_PX\) and \(V^*=V_P^*=T_P^*X\).  Define
\[
  E_P=V\oplus V^*
\]
and
...
\]
whose symplectic pairings vanish.  This isotropy condition is exactly what is
needed for the corresponding Weyl operators to commute, up to the central
phases fixed by the representation convention.  After the selection principle
also fixes eigenvalue phases, the selected stabilizer subspace is the joint
eigenspace inside the carrier of \(\rho_S\).
...
\[
\begin{array}{ll}
1. & \text{Choose }X=\Spec A.\\
2. & \text{Compute points }P\text{ and residue fields }\kappa(P).\\
3. & \text{Compute }T_PX\text{ and }T_P^*X.\\
4. & \text{Build }E_P=T_PX\oplus T_P^*X.\\
...
7. & \text{Take the joint eigenspace of the selected commuting operators}.
\end{array}
\]
