...
If one wants a Fock-space operator whose one-particle restriction is
...
operators for cell fermions. The second-quantized cellular differential is
\[
...
\]
It preserves fermion number and raises cell degree. Its square is zero because
\(\delta^1_{\mathbb Z}\delta^0_{\mathbb Z}=0\), and on the one-particle sector
\(W\) it is exactly the signed cellular cochain differential. This is a
cell-fermion Fock space, not the check-ghost Fock space used by
\(Q_{\mathrm{gh}}\). Reducing the signed incidence matrices modulo \(2\) gives
the CSS supports used below.
