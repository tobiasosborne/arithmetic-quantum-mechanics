...
\]
That is, \(L\) is isotropic. If \(\dim L=r\), the joint eigenspace of the
corresponding independent stabilizers has dimension
\[
  p^{n-r}.
\]
...
The validation script is \path{symplectic_css_bridge_validation.jl} under
\path{scripts/lattice_codes/}. It builds oriented boundary matrices for the
\(k=4\) periodic square torus over \(\mathbb F_p\) for \(p=2,3,5\). It
verifies:
\[
...
\[
  ...
  \dim H_1=32-15-15=2.
\]
The corresponding stabilizer subspace has rank \(30\), so the symplectic count
gives \(32-30=2\) encoded qudits and Hilbert dimension \(p^2\). The generated
CSV is \path{symplectic_css_bridge_summary.csv} in the
\path{2026-05-24-symplectic-css-bridge} run bundle.
