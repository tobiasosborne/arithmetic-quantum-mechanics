...
% SHARD-SUMMARY: Separates all-map, finite-support, sheaf-section, localized regular-function, reduced-smearing, and rational-truncation choices using only local anchors.
...
For \(X=\Spec R\), \(R=\mathbb F_q[x]\), \(U=D(s)\) gives
\(\Gamma(U,\mathcal O_X)=R_s\); a separate \(h\ne0\) may select \(Z_{\rm cl}(h)\). &
\texttt{Gap} for nonzero \(R_s^2\) profiles as finite operators;
\texttt{Checked} for reduced polynomial smearings. &
Localized profiles evaluate only where \(\pi\nmid s\); finite support
\(Z_{\rm cl}(h)\) uses \(P,Q\in R\) modulo \(\operatorname{rad}(h)\). &
...
\textbf{Review.}  The affine-line regular-function branch in AQM-49--AQM-52 is a
structure-sensitive replacement for a vague Hom-space.  Put \(R=\mathbb F_q[x]\).
On \(U=D(s)\), \(s\ne0\), a pair
\((F,G)\in R_s^2\) gives a residue profile at closed points \(x_\pi\in U\),
equivalently \(\pi\nmid s\):
\[
  \operatorname{ev}_U(F,G)_\pi=(F(x_\pi),G(x_\pi))\in\kappa(x_\pi)^2.
\]
Denominators are invertible there; this localized datum is not evaluated at
closed points with \(\pi\mid s\).  By AQM-49 and convention \texttt{(ak)}, a
nonzero profile on a nonempty standard open has cofinite closed support in that
open, so it is formal infinite-volume data, not a quasi-local Weyl operator.

\textbf{Checked.}  A separate nonzero polynomial \(h\in R\) may instead select the finite closed support
\[
  Z_{\rm cl}(h)=\{x_\pi:\pi\mid h\}.
\]
AQM-49's reduced smearing over this support uses polynomial labels
\((P,Q)\in R^2\), equivalently classes modulo \(\operatorname{rad}(h)\):
\[
  W_h^{\rm red}(P,Q)=
  W_{Z_{\rm cl}(h)}\bigl((P\bmod\pi,\ Q\bmod\pi)_{\pi\mid h}\bigr).
\]
It does not evaluate arbitrary localized elements where denominators vanish.
AQM-50 checks this over \(\mathbb F_3\), including rational two-qutrit
supports, a quadratic \(9\)-level site, and repeated factors invisible to the
reduced net.  AQM-51--AQM-52 and convention \texttt{(al)} check that affine
symmetries move profiles, supports, and reduced smearings compatibly while
